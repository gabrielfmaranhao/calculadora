...
const Home = () => {
    ...
    const postApi = async (value:IRequestApi) => {
        try {
            ...
            { error:"Erro interno do servidor", pending:"Por favor aguarde" } 
         )
        } catch (error) {
            console.log(error)
        }
    ...
    return(
        ...
        transition = {{ duration: 0.5 }}>
            <Container>
                ...
                {response ?
                <motion.div className="motion-div-response"
                initial    = {{scale:0}}
                animate    = {{ scale: 1  }}
                transition = {{ duration: 0.8 }}
                >
                    <BoxResponse className="response">
                        <Text element="h3" li_Heigt={20} size={20} color="blue-2" title="VOCÊ RECEBERÁ:"/>
                        <ul>
                            <li>
